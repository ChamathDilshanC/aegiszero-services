    ...
    public SecurityFilterChain filterChain(HttpSecurity http,
        ...
        http
                ...
                .authorizeHttpRequests(auth -> auth
                        .requestMatchers("/api/security/internal/**", "/actuator/health", "/actuator/info").permitAll()
                        .anyRequest().authenticated()
                )
                .addFilterBefore(correlationIdFilter, UsernamePasswordAuthenticationFilter.class)
                // permitAll above only means "don't require a user JWT" — this filter
                // is what actually locks /api/security/internal/** down to callers
                // that know the shared secret, since these endpoints have no other guard.
